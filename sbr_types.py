...
#This function performs certain functions in other modules
def valid_argument_type(name, args=(), args_types=(), input_data=None, input_types=None):
    if input_data is not None and input_types is not None:
        if not isinstance(input_data, input_types):
            raise SBR_ERROR(
                f"{name} does'nt accept this type of data as input '{type(input_data).__name__}'")
    for arg in args:
        if not isinstance(arg, args_types):
            raise SBR_ERROR(
                f"{name} doesn't accept this type of data as argument '{type(arg).__name__}'")
